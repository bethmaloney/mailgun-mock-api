...
# Each iteration: orchestrator selects a task, then spawns sub-agents for:
#   1. Test writing (TDD - tests first)
#   2. Implementation (make tests pass)
#   3. Code review (flag issues by severity)
#   4. Fix critical/high issues (conditional)
# Exits early if "ALL TODO ITEMS COMPLETE" is detected

set -e

PROMPT_FILE="scripts/IMPLEMENTATION_PROMPT.md"
MAX_ITERATIONS=40
ITERATION=0
...
echo "Workflow: orchestrator → test writer → implementer → reviewer → fixer (if needed)"
echo ""
...
while true; do
    ...
    # The orchestrator reads the plan, selects a task, then spawns sub-agents internally
    ...
    # Run lint (Go vet + frontend ESLint via justfile)
    # This is a safety net — the sub-agents should have already run go vet
    echo -e "\n------------------------ LINT CHECK ------------------------"
    LINT_RETRIES=0
    MAX_LINT_RETRIES=3
    LINT_PASSED=false
