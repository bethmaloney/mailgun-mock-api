...
# Script to repeatedly run Claude Code with E2E_TESTING_PROMPT.md
# Each iteration: orchestrator selects one spec section, then spawns sub-agents for:
#   1. Test writer (create Playwright tests for that section)
#   2. Fixer (if tests fail, fix tests or app code)
#   3. Reviewer (audit tests, flag issues by severity)
#   Main agent fixes any CRITICAL/HIGH issues from reviewer
# Exits early if "ALL TODO ITEMS COMPLETE" is detected

set -e

PROMPT_FILE="scripts/E2E_TESTING_PROMPT.md"
MAX_ITERATIONS=30
ITERATION=0
...
echo "Starting E2E testing loop on branch: $CURRENT_BRANCH"
